...
if __name__ == '__main__':
    ...
    def test_sem_to_v2_alignment1():
        ...
        sem_path = os.path.join(SHAPENET_SEM_ROOT, 'models-OBJ', 'models', f'{id}.obj')
        sem_mesh = normalize_mesh(o3d.io.read_triangle_mesh(sem_path)).compute_triangle_normals()
        x_rot = R.from_rotvec([-90, 0, 0], degrees=True).as_matrix()
        y_rot = R.from_rotvec([0, 180, 0], degrees=True).as_matrix()
        sem_mesh.rotate(x_rot)
        sem_mesh.rotate(y_rot)
        
        v2_path = SHAPENETV2_MODEL_PATH_TEMPLATE.format(shapenet_id=id, shapenet_cat_id=v2_cat_id)
        v2_mesh = o3d.io.read_triangle_mesh(v2_path).paint_uniform_color([1, 0, 0]).compute_triangle_normals()
        sem_pc = sem_mesh.sample_points_uniformly(1000)
        v2_pc = v2_mesh.sample_points_uniformly(1000)
        trans = get_icp_between_pointclouds(sem_pc, v2_pc, use_normal=True, max_iteration=20000)
        sem_mesh.transform(trans)

        o3d.visualization.draw_geometries([sem_mesh, v2_mesh, o3d.geometry.TriangleMesh.create_coordinate_frame()])
    ...
    def test_sem_alignment():
        ...
        sem_path1 = os.path.join(SHAPENET_SEM_ROOT, 'models-OBJ', 'models', f'{sem_id1}.obj')
        sem_path2 = os.path.join(SHAPENET_SEM_ROOT, 'models-OBJ', 'models', f'{sem_id2}.obj')
        mesh1 = normalize_mesh(o3d.io.read_triangle_mesh(sem_path1).paint_uniform_color([1, 0, 0])).compute_triangle_normals()
        mesh2 = normalize_mesh(o3d.io.read_triangle_mesh(sem_path2).paint_uniform_color([0, 1, 0])).compute_triangle_normals()
    ...
    def test_sem_shapenetpart_reg():
        ...
        sem_path = os.path.join(SHAPENET_SEM_ROOT, 'models-OBJ', 'models', f'{shape_id}.obj')
        sem_mesh = normalize_mesh(o3d.io.read_triangle_mesh(sem_path))
        x_rot = R.from_rotvec([-90, 0, 0], degrees=True).as_matrix()
        sem_mesh.rotate(x_rot)
        shapenetpart_path = os.path.join(SHAPENET_PART_DATA_ROOT, part_cat_id, 'points', f'{shape_id}.pts')
        shapenetpart_pnt = o3d.utility.Vector3dVector(read_shapenetpart_points(shapenetpart_path))
        shapenetpart_pcd = o3d.geometry.PointCloud()
        shapenetpart_pcd.points = shapenetpart_pnt
        o3d.visualization.draw_geometries([sem_mesh, shapenetpart_pcd, o3d.geometry.TriangleMesh.create_coordinate_frame()])
        pass
    ...
    def test_sem_v1_alig():
        ...
        with open(v1_meta_path, 'r') as f:
            ...
            for row in reader:
                ...
                cur_id = row['fullId'][5:]
                cur_meta = {}
                cur_meta['up'] = [float(ele) for ele in row['up'].split('\,')] if row['up'] else None
                cur_meta['front'] = [float(ele) for ele in row['front'].split('\,')] if row['front'] else None
